...
function DexSearch(){var searchType=arguments.length>0&&arguments[0]!==undefined?arguments[0]:'';var formatid=arguments.length>1&&arguments[1]!==undefined?arguments[1]:'';var species=arguments.length>2&&arguments[2]!==undefined?arguments[2]:'';this.query='';this.dex=Dex;this.typedSearch=null;this.results=null;this.prependResults=null;this.resultsComponent=null;this.selection=0;this.exactMatch=false;this.firstPokemonColumn='Number';this.numAbilityCols=0;this.sortCol=null;this.reverseSort=false;this.filters=null;
this.setType(searchType,formatid,species);
}DexSearch.getTypeName=function getTypeName(type){switch(type){case'pokemon':return TL(["Pok\xE9mon"]);case'type':return TL(["Types"]);case'move':return TL(["Moves"]);case'item':return TL(["Items"]);case'ability':return TL(["Abilities"]);case'egggroup':return TL(["Egg Groups"]);case'category':return TL(["Categories"]);case'tier':return TL(["Tiers"]);case'article':return TL(["Article"]);default:return DexSearch.typeName[type];}};var _proto=DexSearch.prototype;_proto.
...
getFirstResultIndex=function getFirstResultIndex(){
...
moveSelection=function moveSelection(offset){var _this$results,_this$resultsComponen,_this$resultsComponen2;
if(!((_this$results=this.results)!=null&&_this$results.length))return false;
...
(_this$resultsComponen=this.resultsComponent)==null||_this$resultsComponen.scrollSelectedResult();
(_this$resultsComponen2=this.resultsComponent)==null||_this$resultsComponen2.updateSelection();
return true;
};_proto.

selectResult=function selectResult(){var _this$results2;var index=arguments.length>0&&arguments[0]!==undefined?arguments[0]:this.selection;
var result=(_this$results2=this.results)==null?void 0:_this$results2[index];
while(result&&DexSearch.unselectableResultTypes.includes(result[0])){var _this$results3;
index++;
result=(_this$results3=this.results)==null?void 0:_this$results3[index];
}
...
getResultName=function getResultName(result){
switch(result[0]){
case'pokemon':
...
var bufs=[[],[],[],[],[],[],[],[],[],[]];
var seenInBuf=bufs.map(function(){return Object.create(null);});
var topbufIndex=-1;
...
bufs[typeIndex]=[['header',TL(["Illegal Pok\xE9mon"])]];
}
}
}else{
if(!bufs[typeIndex].length){
...
var typeName=TL.type[fId]||type;
buf.push(['header',TL(["","-type Pok\xE9mon"],typeName)]);
...
var abilityName=TL(Dex.abilities.get(fId));
buf.push(['header',TL([""," Pok\xE9mon"],abilityName)]);
...
var _typeName=TL.type[fId]||_type;
buf.push(['header',TL(["","-type moves"],_typeName)]);
...
var categoryName=TL.tag[fId]||category;
buf.push(['header',TL([""," moves"],categoryName)]);
...
return left;
};return DexSearch;}();DexSearch.typeTable={pokemon:1,type:2,tier:3,move:4,item:5,ability:6,egggroup:7,category:8,article:9};DexSearch.typeName={pokemon:"Pok\xE9mon",type:'Type',tier:'Tiers',move:'Moves',item:'Items',ability:'Abilities',egggroup:'Egg group',category:'Category',article:'Article'};DexSearch.unselectableResultTypes=['header','html','sortpokemon','sortmove'];var
...
if(format.startsWith('natdex'))format=format.slice(6);
if(format.startsWith('nationaldex'))format=format.slice(11);
if(format.startsWith('vgc')||format.startsWith('bss'))format='ubers';
if(format.endsWith('draft'))format='ag';
}
...
results=[].concat(results,[['header',TL(["Illegal results"])]],illegalResults);
}
return results;
};_proto2.
...
canLearn=function canLearn(speciesid,moveid){var _this$formatType2,_this$formatType3;
var move=this.dex.moves.get(moveid);
if((_this$formatType2=this.formatType)!=null&&_this$formatType2.includes('natdex')&&move.isNonstandard&&move.isNonstandard!=='Past'){
return false;
}
...
this.dex.gen===9&&!((_this$formatType3=this.formatType)!=null&&_this$formatType3.includes('natdex')))
{
if(gen===9){
genChar='a';
}else if(gen===8){
...
while(learnsetid){var _this$formatType4;
var table=BattleTeambuilderTable;
if((_this$formatType4=this.formatType)!=null&&_this$formatType4.startsWith('bdsp'))table=table['gen8bdsp'];
if(this.formatType==='letsgo')table=table['gen7letsgo'];
if(this.formatType==='bw1')table=table['gen5bw1'];
if(this.formatType==='rs')table=table['gen3rs'];
if(this.formatType==='frlg')table=table['gen3frlg'];
...
this.formatType==='natdexchampions'?"natdexchampions":"gen"+
gen;
...
var genHeader=function(num){return TL(["Generation ",""],num);};
var results=[];
for(var id in BattlePokedex){
switch(id){
case'bulbasaur':
...
var isDoublesOrBS=isVGCOrBS||((_this$formatType5=this.formatType)==null?void 0:_this$formatType5.includes('doubles'));
var dex=this.dex;
...
this.formatType!=='svdlc1doubles'&&!((_this$formatType6=this.formatType)!=null&&_this$formatType6.includes('natdex'))&&(
...
table=table["gen"+dex.gen];
}else if((_this$formatType7=this.formatType)!=null&&_this$formatType7.startsWith('bdsp')){
...
}else if((_this$formatType8=this.formatType)!=null&&_this$formatType8.startsWith('ssdlc1')){
if(this.formatType.includes('doubles')){
table=table['gen8dlc1doubles'];
}else{
table=table['gen8dlc1'];
}
}else if((_this$formatType9=this.formatType)!=null&&_this$formatType9.startsWith('predlc')){
if(this.formatType.includes('doubles')){
...
}else if((_this$formatType10=this.formatType)!=null&&_this$formatType10.startsWith('svdlc1')){
if(this.formatType.includes('doubles')){
...
}else if(isVGCOrBS||isHackmons&&dex.gen===9&&!this.formatType){var _this$formatType11;
...
if((_this$formatType11=this.formatType)!=null&&_this$formatType11.includes('dlc')){
...
if(format==='pu')tierSet=tierSet.slice(slices.PU);else
if(format==='zu')tierSet=tierSet.slice(slices.ZU);else
if(
...
if((_this$formatType12=this.formatType)!=null&&_this$formatType12.startsWith('bdsp')||this.formatType==='letsgo'||this.formatType==='stadium'){
tierSet=tierSet.slice(slices.Uber);
...
var abilitySet=[['header',TL(["Abilities"])]];
...
getDefaultResults=function getDefaultResults(){var _this$formatType13,_this$formatType14;
var table=BattleTeambuilderTable;
if((_this$formatType13=this.formatType)!=null&&_this$formatType13.startsWith('bdsp')){
...
}else if((_this$formatType14=this.formatType)!=null&&_this$formatType14.endsWith('doubles')){
...
return[
['header',TL(["Specific to ",""],TL(this.dex.species.get(speciesName)))]].concat(
speciesSpecific,
results);
...
return[
['header',TL(["Specific to ",""],TL(this.dex.abilities.get(this.set.ability)))]].concat(
abilitySpecific,
results);
...
moveIsNotUseless=function moveIsNotUseless(id,species,moves,set){var _this$formatType15;
...
case'dazzlinggleam':
return!moves.includes('alluringvoice')||((_this$formatType15=this.formatType)==null?void 0:_this$formatType15.includes('doubles'));
case'darkvoid':
return dex.gen<7;
case'dualwingbeat':
...
getBaseResults=function getBaseResults(){var _this$formatType16,_this$formatType17,_this$formatType18,_this$formatType19,_this$formatType20;
if(!this.species)return this.getDefaultResults();
var dex=this.dex;
var species=dex.species.get(this.species);
var format=this.format;
...
format.startsWith('vgc')||dex.gen===9&&!((_this$formatType16=this.formatType)!=null&&_this$formatType16.includes('natdex')));
...
if((_this$formatType17=this.formatType)!=null&&_this$formatType17.startsWith('bdsp'))lsetTable=lsetTable['gen8bdsp'];
...
if((_this$formatType18=this.formatType)!=null&&_this$formatType18.startsWith('ssdlc1'))lsetTable=lsetTable['gen8dlc1'];
if((_this$formatType19=this.formatType)!=null&&_this$formatType19.startsWith('predlc'))lsetTable=lsetTable['gen9predlc'];
if((_this$formatType20=this.formatType)!=null&&_this$formatType20.startsWith('svdlc1'))lsetTable=lsetTable['gen9dlc1'];
while(learnsetid){
...
if(!((_this$formatType21=this.formatType)!=null&&_this$formatType21.includes('natdex'))&&move.isNonstandard==="Past"){
continue;
}
if(
...
if(_move.gen>dex.gen||!_move.exists)continue;
if(sketch){
