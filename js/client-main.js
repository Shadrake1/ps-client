...
var NARROW_MODE_HEADER_WIDTH=280;
...
_this=_PSStreamModel.call(this)||this;_this.theme='light';_this.nogif=null;_this.noanim=null;_this.bwgfx=null;_this.nopastgens=null;_this.inchatpm=null;_this.noselfhighlight=null;_this.temporarynotifications=null;_this.leavePopupRoom=null;_this.refreshprompt=null;_this.chatformatting={hidegreentext:false,hideme:false,hidespoiler:false,hidelinks:false,hideinterstice:true};_this.nounlink=null;_this.ignorenicks=null;_this.ignorespects=null;_this.ignoreopp=null;_this.autotimer=null;_this.autohardcore=null;_this.spectatefromstart=null;_this.battlelayout=null;_this.rightpanelbattles=null;_this.disallowspectators=null;_this.starredformats=null;_this.teameditorspacious=null;_this.teameditorzoomoutforms=null;_this.teameditorzoomoutsearch=null;_this.showjoins=null;_this.showdebug=null;_this.showbattles=true;_this.autojoin=null;_this.ignore=null;_this.tournaments=null;_this.onepanel=false;_this.timestamps={};_this.mute=false;_this.effectvolume=50;_this.musicvolume=50;_this.notifvolume=50;_this.uploadprivacy=true;_this.afd=undefined;_this.highlights=null;_this.logtimes=null;_this.avatar=null;_this.serversettings={};_this.storageEngine='';_this.storage={};_this.origin="https://"+Config.routes.client;
...
setShowDebug=function setShowDebug(){var mode=arguments.length>0&&arguments[0]!==undefined?arguments[0]:this.showdebug;
var css=mode?".debug {display: block;}":".debug {display: none;}";
...
PS.addRoom({id:id,title:title,connected:'pending',autofocus:false});
}
var cmd="/autojoin "+rooms;


if((_PS$connection=PS.connection)!=null&&_PS$connection.queue.includes(cmd))return;
PS.send(cmd);
}
...
spliceIn=function spliceIn(index,teams){var _this$list;for(var _i6=0;_i6<
teams.length;_i6++){var _team2=teams[_i6];
_team2.key||(_team2.key=this.getKey(_team2.name));
this.byKey[_team2.key]=_team2;
}
(_this$list=this.list).splice.apply(_this$list,[index,0].concat(teams));
};_proto2.
unpackOldBuffer=function unpackOldBuffer(buffer){
var upgradeUrl="https://"+Config.routes.client+"/recoverteams.html";
...
var teams={};for(var _i8=0,_data$teams2=
data.teams;_i8<_data$teams2.length;_i8++){var _team3=_data$teams2[_i8];
teams[_team3.teamid]=_team3;
}

var NOT_LOADED_REGEX=/^[^|]*\|\|\|\|\|\|\|\|\|\|\|(?:\][^|]*\|\|\|\|\|\|\|\|\|\|\|)*$/;for(var _i10=0,_this3$list2=

_this3.list;_i10<_this3$list2.length;_i10++){var localTeam=_this3$list2[_i10];
if(localTeam.teamid){
var _team4=teams[localTeam.teamid];
if(!_team4){
continue;
}
localTeam.uploaded={
...
}for(var _i12=0,_Object$values2=


Object.values(teams);_i12<_Object$values2.length;_i12++){var _team5=_Object$values2[_i12];
var matched=false;for(var _i14=0,_this3$list4=
_this3.list;_i14<_this3$list4.length;_i14++){var _localTeam=_this3$list4[_i14];
if(_localTeam.teamid)continue;

var compare=_this3.compareTeams(_team5,_localTeam);
if(compare==='rename'){
...
matched=true;
_localTeam.teamid=_team5.teamid;
_localTeam.uploaded={
teamid:_team5.teamid,
notLoaded:NOT_LOADED_REGEX.test(_localTeam.packedTeam),
"private":_team5["private"]
};
break;
}
}
...
var newTeam={
name:_team5.name,
format:_team5.format,
folder:'',
packedTeam:_this3.unloadedPackedTeam(_team5.team),
iconCache:null,
isBox:false,
key:_this3.getKey(_team5.name),
teamid:_team5.teamid,
uploaded:{
teamid:_team5.teamid,
notLoaded:true,
...
unloadedPackedTeam=function unloadedPackedTeam(uploadedTeam){
var mons=uploadedTeam.split(',').map(function(m){return{species:m,moves:[]};});
...
var actionError=(data==null?void 0:data.actionerror)||TL(["Error unknown. Try again later."]);
...
for(var _i15 in normalizeList){
normalizeRegexes.push([new RegExp('['+normalizeList[_i15]+']','g'),_i15]);
}for(var _i17=0;_i17<

replaceRegexes.length;_i17++){var _ref2=replaceRegexes[_i17];var regex=_ref2[0];var replacement=_ref2[1];
name=name.replace(regex,replacement);
}for(var _i19=0;_i19<
normalizeRegexes.length;_i19++){var _ref3=normalizeRegexes[_i19];var _regex=_ref3[0];var _replacement=_ref3[1];
...
PS.alert(TL(["Something is interfering with our connection to the login server. Most likely, your internet provider needs you to re-log-in, or your internet provider is blocking Pok\xE9mon Showdown."]));
return;
}
if(assertion===';'){
...
defaultGroupSymbol=' ';}var _proto4=PSServer.prototype;_proto4.
...
'language':function(target){var _Dex$text$findLanguag2;
if(!target)return true;
if(['auto','automatic'].includes(toID(target))){
PS.prefs.set('serversettings',Object.assign({},PS.prefs.serversettings,{language:undefined}));
}else{var _Dex$text$findLanguag;
var language=(_Dex$text$findLanguag=Dex.text.findLanguage(target))==null?void 0:_Dex$text$findLanguag.legacyId;

if(!language)return true;
PS.prefs.set('serversettings',Object.assign({},PS.prefs.serversettings,{language:language}));
}
void Dex.loadTextData().then(function(){return PS.updateTranslatedText();});
return"/language "+(((_Dex$text$findLanguag2=Dex.text.findLanguage(Dex.text.getLanguage()))==null?void 0:_Dex$text$findLanguag2.legacyId)||'english');
},
...
if(room!=null&&room.interruptClose(!!target,elem))return;
...
'open,user':function(target,cmd,elem){
var roomid=toID(target)?"user-"+toID(target):"users";
PS.join(roomid,{
...
if(!rooms.length)return this.add('||You do not have any PM windows open.');for(var _i21=0;_i21<
rooms.length;_i21++){var _roomid4=rooms[_i21];
PS.leave(_roomid4);
}
...
for(var _i22=0,_len2=targets.length;_i22<_len2;_i22++){
if(!targets[_i22])continue;
if(/[\\^$*+?()|{}[\]]/.test(targets[_i22])){

try{
new RegExp(targets[_i22]);
}catch(e){
return this.errorReply(
e.message.startsWith('Invalid regular expression: ')?
e.message:
'Invalid regular expression: /'+targets[_i22]+'/: '+e.message
);
}
}
if(highlightList.includes(targets[_i22])){
return this.errorReply(targets[_i22]+" is already on your highlights list.");
}
}
...
var newHls=[];
for(var _i23=0,_len3=_highlightList.length;_i23<_len3;_i23++){
if(!targets.includes(_highlightList[_i23])){
newHls.push(_highlightList[_i23]);
}
}
...
case'battle':{
var sideBySide=!this.prefs.battlelayout||
...
updateLayout=function updateLayout(){var _log;
...
var roomHeight=viewportHeight-56;
var needsUpdate=this.leftPanelWidth!==leftPanelWidth;
if(leftPanelWidth===null){
var headerWidthOffset=viewportWidth<=700?0:VERTICAL_HEADER_WIDTH;
var roomWidth=viewportWidth+1-headerWidthOffset;
needsUpdate||(needsUpdate=this.roomLayoutBreakpointPassed(this.panel,roomWidth,viewportHeight-30));
this.panel.width=roomWidth;
this.panel.height=viewportHeight-30;
}else if(leftPanelWidth){
var rightPanelWidth=viewportWidth+1-leftPanelWidth;
needsUpdate||(needsUpdate=this.roomLayoutBreakpointPassed(this.leftPanel,leftPanelWidth,roomHeight));
needsUpdate||(needsUpdate=this.roomLayoutBreakpointPassed(this.rightPanel,rightPanelWidth,roomHeight));
...
needsUpdate||(needsUpdate=this.roomLayoutBreakpointPassed(this.panel,viewportWidth,roomHeight));
...
(_log=this.panel.log)==null||_log.updateScroll();
if(this.leftPanelWidth){var _log2,_log3;
(_log2=this.leftPanel.log)==null||_log2.updateScroll();
(_log3=this.rightPanel.log)==null||_log3.updateScroll();
}
return needsUpdate;
};_proto7.
...
updateTranslatedText=function updateTranslatedText(){for(var _i29=0,_Object$values4=
Object.values(this.rooms);_i29<_Object$values4.length;_i29++){var room=_Object$values4[_i29];
var battle=room==null?void 0:room.battle;
if(battle)battle.resetToCurrentTurn();
}
this.update();
};_proto7.
receive=function receive(msg){var _room4;
msg=msg.endsWith('\n')?msg.slice(0,-1):msg;
var roomid='';
if(msg.startsWith('>')){
...
var isInit=false;for(var _i31=0,_msg$split2=
msg.split('\n');_i31<_msg$split2.length;_i31++){var _room3;var line=_msg$split2[_i31];
...
continue;
}case'noinit':{var _room,_room2;
room=PS.rooms[roomid2];
var roomPreviouslyConnected=((_room=room)==null?void 0:_room.connected)===true||((_room2=room)==null?void 0:_room2.connectMode)==='pending-reconnect';
...
(_room3=room)==null||_room3.receiveLine(args);
}
if(room&&isInit)room.connected=true;
(_room4=room)==null||_room4.update(isInit?["initdone"]:null);
};_proto7.
...
isVisiblePanel=function isVisiblePanel(room){
...
var room=new Model(options);
room.location=this.normalizeRoomLocation(room.location);
return room;
};_proto7.


normalizeRoomLocation=function normalizeRoomLocation(location){
...
addRoomType=function addRoomType(){for(var _len4=arguments.length,types=new Array(_len4),_key5=0;_key5<_len4;_key5++){types[_key5]=arguments[_key5];}for(var _i33=0;_i33<
types.length;_i33++){var RoomType=types[_i33];
this.roomTypes[RoomType.id]=RoomType;for(var _i35=0,_RoomType$routes2=
RoomType.routes;_i35<_RoomType$routes2.length;_i35++){var route=_RoomType$routes2[_i35];
...
var updated=false;
for(var _roomid5 in this.rooms){
var room=this.rooms[_roomid5];
var typeIsGuessed=room.type===this.routes['*']&&!_roomid5.includes('-');
if(!room.isPlaceholder&&!typeIsGuessed)continue;

var type=!typeIsGuessed&&room.type||this.getRoute(_roomid5)||room.type||'';
...
if(_roomid5==='')this.mainmenu=newRoom;
if(this.room===room){
this.room=newRoom;
...
setFocus=function setFocus(room,options){
room.onRequestFocus==null||room.onRequestFocus(options);
};_proto7.
focusRoom=function focusRoom(roomid){
var room=this.rooms[roomid];
if(!room)return false;
if(this.room===room){
var focusOptions=room.focusNextUpdate===true?undefined:room.focusNextUpdate||undefined;
...
fullNav=function fullNav(){var room=arguments.length>0&&arguments[0]!==undefined?arguments[0]:this.room;
var rooms=[''].concat(this.miniRoomList,this.leftRoomList.slice(1),this.rightRoomList);
var index=rooms.indexOf(room.id);
return{rooms:rooms,index:index};
};_proto7.
...
focusUnreadRoom=function focusUnreadRoom(direction){
var _this$fullNav=this.fullNav(),rooms=_this$fullNav.rooms,index=_this$fullNav.index;
if(index===-1)return;

var unreadRooms=rooms.filter(function(room,i){var _PS$rooms$room,_PS$rooms$room2;return(
(((_PS$rooms$room=PS.rooms[room])==null?void 0:_PS$rooms$room.isSubtleNotifying)||((_PS$rooms$room2=PS.rooms[room])==null?void 0:_PS$rooms$room2.notifications.length))&&(
direction==='left'?i<index:i>index));}
);

if(!unreadRooms.length)return;

var target=direction==='left'?unreadRooms[unreadRooms.length-1]:unreadRooms[0];
...
(_opts$cancelButton=opts.cancelButton)!=null?_opts$cancelButton:opts.cancelButton=TL(["[Cancel]"]);
...
(_opts$cancelButton2=opts.cancelButton)!=null?_opts$cancelButton2:opts.cancelButton=TL(["[Cancel]"]);
return new Promise(function(resolve){
...
options.args=Object.assign({challengeMenuOpen:true},options.args);
}
...
if(preexistingRoom){var _options$args;
if((_options$args=options.args)!=null&&_options$args.format){
preexistingRoom.args=options.args;
if(preexistingRoom.challengeMenuOpen){
options.args.format="!!"+options.args.format;
}
}
if(options.autofocus){var _options$args2;
if((_options$args2=options.args)!=null&&_options$args2.challengeMenuOpen){
...
if(options.backlog){for(var _i37=0,_options$backlog2=
options.backlog;_i37<_options$backlog2.length;_i37++){var args=_options$backlog2[_i37];
room.receiveLine(args);
}
}
...
background!=null?background:background=!this.isVisible(room);
...
if(this.popups[i]===(room==null?void 0:room.id))break;
...
var autojoins=[];
var autojoinCount=0;
var rooms=[].concat(this.miniRoomList,this.leftRoomList,this.rightRoomList);for(var _i39=0;_i39<
rooms.length;_i39++){var _roomid6=rooms[_i39];
var room=PS.rooms[_roomid6];
if(!room)return;
