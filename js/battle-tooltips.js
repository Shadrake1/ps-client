...
getDisplayedTarget=function getDisplayedTarget(move){
switch(move.target){
...
var _moveName=BattleTooltips.zMoveTable[item.zMoveType];
var zMove=this.battle.dex.moves.get(_moveName);
...
basePowers.push(TL.label(TL(["Base power vs. ",""],active.name),_basePower));
}
if(difference){
...
var calledMove=Dex.getTypeIcon(this.getMoveType(called,value)[0])+" "+TL(called);
...
var priority=move.id==='grassyglide'&&this.battle.hasPseudoWeather('Grassy Terrain')?1:move.priority;
if(priority>1){
text+=TL(["Nearly always moves first (priority +",")."],priority)+'</p><p>';
}else if(priority<=-1){
var absPriority=-priority;
...
var displayedTarget=this.getDisplayedTarget(move);
if(displayedTarget){
...
var hpList=hpValues.join(TL([" or "]));
var failMessage=hpValues.length?TL(["Fails if current HP is ","."],hpList):'';
...
var _failMessage=selfKO?TL(["KOs yourself if current HP is exactly ","."],selfKO):'';
...
var target=BattleLog.escapeHTML(this.getNickname(possibleTarget));
if(effectiveness===null){

}else if(effectiveness===0){
var effect=TL(["No effect"]);
text+="<p><span class=\"effectiveness-icon\">&times;</span> "+TL(["<strong>","</strong> vs. ",""],effect,target)+"</p>";
}else if(effectiveness<0.5){
var effectivenessText=effectiveness===0.25?'&#x00BC;':effectiveness;
var _effect=TL(["Mostly ineffective"]);
text+="<p><span class=\"effectiveness-icon\">&#x25BC;</span> "+TL(["<strong>","</strong> vs. ",""],_effect,target)+" <small>("+effectivenessText+"&times;)</small></p>";
}else if(effectiveness<1){
var _effectivenessText=effectiveness===0.5?'&#x00BD;':effectiveness;
var _effect2=TL(["Not very effective"]);
text+="<p><span class=\"effectiveness-icon\">&#x25B3;</span> "+TL(["<strong>","</strong> vs. ",""],_effect2,target)+" <small>("+_effectivenessText+"&times;)</small></p>";
}else if(effectiveness>2){
var _effect3=TL(["Extremely effective"]);
text+="<p><span class=\"effectiveness-icon\">&#x2605;</span> "+TL(["<strong>","</strong> vs. ",""],_effect3,target)+" <small>("+effectiveness+"&times;)</small></p>";
}else if(effectiveness>1){
var _effect4=TL(["Super effective"]);
text+="<p><span class=\"effectiveness-icon\">&#x29BF;</span> "+TL(["<strong>","</strong> vs. ",""],_effect4,target)+" <small>("+effectiveness+"&times;)</small></p>";
}
}
...
var species=this.battle.dex.species.get(pokemon.speciesForme);
...
var genderName=TL.gender[gender==='M'?'male':'female']||gender;
...
var speciesName=TL(this.battle.dex.species.get(pokemon.speciesForme));
if(speciesName!==nickname){

var speciesSuffix=BattleTextParser.defaultText('fullName',{NICKNAME:'',SPECIES:speciesName});
name+="<small>"+speciesSuffix+"</small>";
}

var levelBuf=pokemon.level!==100?" <small>"+TL(["Level"])+" "+pokemon.level+"</small>":"";
...
var forme=TL(this.battle.dex.species.get(clientPokemon.volatiles.formechange[1]));
...
if(pokemon.terastallized){
var baseTypes="<span class=\"textaligned-typeicons\">"+this.getPokemonTypes(pokemon,true).map(function(type){return Dex.getTypeIcon(type);}).join(' ')+"</span>";
text+="&nbsp; &nbsp; <small>"+TL(["(base: ",")"],baseTypes)+"</small>";
}else if(knownPokemon.teraType){
var teraType="<span class=\"textaligned-typeicons\">"+Dex.getTypeIcon(knownPokemon.teraType)+"</span>";
var teraTypeText=TL(["Tera ",""],teraType);
...
var hpName=TL.stat.hp||'HP';
var faintedName=TL.status.fnt||'fainted';
var statusName=pokemon.status?TL.status[pokemon.status]||pokemon.status.toUpperCase():'';
var status=pokemon.status?" <span class=\"status "+
pokemon.status+"\">"+statusName+"</span>":'';
...
item='None';
var prevItem=TL(this.battle.dex.items.get(clientPokemon.prevItem));
...
var _prevItem=TL(this.battle.dex.items.get(clientPokemon.prevItem));
_itemEffect+=clientPokemon.prevItemEffect?_prevItem+' was '+clientPokemon.prevItemEffect:'was '+_prevItem;
}
if(pokemon.item)_item=TL(this.battle.dex.items.get(pokemon.item));
if(_itemEffect)_itemEffect=' ('+_itemEffect+')';
if(_item)itemText="<small>"+TL.label(TL(["Item"]))+"</small>"+_item+_itemEffect;
}
...
var nature=TL.nature[toID(clientPokemon.nature)]||clientPokemon.nature;
var natureText="<small>"+TL.label(TL(["Nature"]))+"</small>"+nature;
text+="<p>"+natureText+"</p>";
}
...
clientPokemon.moveTrack;_i16<_clientPokemon$moveTr2.length;_i16++){var _ref2=_clientPokemon$moveTr2[_i16];var _moveName2=_ref2[0];{
var _move=this.battle.dex.moves.get(_moveName2);
text+="&#8226; "+TL(_move)+"<br />";
}}
text+="</p>";
...
text+=TL(["(Your opponent has two indistinguishable Pok\xE9mon, making it impossible for you to tell which one has which moves/ability/item.)"])+" ";
}
text+="</p>";
}
return text;
...
var weatherbuf=scene.weatherLeft()||TL(["(no weather)"]);
...
var speedName=TL.statShort.spe||'Spe';
var speedRange=TL([""," to ",""],min,max);
if(this.battle.gen<3){
var beforeStatStages=TL(["(before stat stage changes)"]);
...
var beforeExternalModifiers=TL(["(before external modifiers)"]);
if(this.battle.tier.includes('Random')){
...
buf+=(TL.statShort[_statLabel]||_statLabel)+"&nbsp;</small>";
...
var low=maxpp-ppUsed[0]+"/"+maxpp;
var high=maxpp-ppUsed[1]+"/"+maxpp;
var range=TL(["("," to ",")"],low,high);
...
var abilityName=TL(this.battle.dex.abilities.get(abilityData.ability));
text="<small>"+TL.label(TL(["Ability"]))+"</small>"+abilityName;
var baseAbilityName=TL(this.battle.dex.abilities.get(abilityData.baseAbility));
...
var hp=evs['hp']||0;
var hpStat=_this5.getStat('hp',set,hp,1);
...
hp+=step;
hpStat=_this5.getStat('hp',set,hp,1);
currentEVTotal+=step;
}
...
while(hp>0&&hpStat===_this5.getStat('hp',set,hp-step,1)){
hp-=step;
currentEVTotal-=step;
}
...
var origLeftoverEVs=useStatPoints?66:508;for(var _i72=0,_Dex$statNames2=
Dex.statNames;_i72<_Dex$statNames2.length;_i72++){var _origSpread$evs;var stat=_Dex$statNames2[_i72];
...
if(!minusTooLow){for(var _i74=0,_Dex$statNamesExceptH14=
Dex.statNamesExceptHP;_i74<_Dex$statNamesExceptH14.length;_i74++){var _stat=_Dex$statNamesExceptH14[_i74];
...
if(!plusTooHigh){for(var _i76=0,_Dex$statNamesExceptH16=
Dex.statNamesExceptHP;_i76<_Dex$statNamesExceptH16.length;_i76++){var _stat2=_Dex$statNamesExceptH16[_i76];
...
}for(var _i78=0,_Dex$statNames4=
Dex.statNames;_i78<_Dex$statNames4.length;_i78++){var _stat3=_Dex$statNames4[_i78];
...
if(savedEVs<0)return null;for(var _i80=0,_Dex$statNames6=
Dex.statNames;_i80<_Dex$statNames6.length;_i80++){var _stat4=_Dex$statNames6[_i80];
